...
namespace Gerenciador.Controllers
{
    ...
    public class FileController : Controller
    {
       private readonly Contexto _context;
        ...
        {
            var recebedados = new FileUploadViewModel();
            recebedados.FilesOnDatabase = await _context.FilesOnDatabase.ToListAsync();
            return View(recebedados);
            //return View(await _context.FilesOnDatabase.ToListAsync()); 
        }
        ...
        [Authorize(Roles = "Administrador, Aluno")]
        [HttpPost]
        public async Task<IActionResult> UploadToDatabasePrivate(List<IFormFile> files, string description)
        {
            foreach (var file in files)
            {
                var fileName = Path.GetFileNameWithoutExtension(file.FileName);
